...
#define GATTS_CHAR1_UUID          0xFF01
#define GATTS_DESCR1_UUID         0x3333
#define GATTS_CHAR2_UUID          0xFF02
#define GATTS_DESCR2_UUID         0x3334
...
static esp_gatt_char_prop_t read_property = 0;
static esp_gatt_char_prop_t write_property = 0;

uint8_t *ble_prop_buf = NULL;
uint16_t ble_prop_buf_len = 0;
ble_server_write_cb_t ble_write_cb = NULL;
ble_server_read_cb_t ble_read_cb = NULL;

static uint8_t write_buf[] = {0x00, 0x01};
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_WRITE_EVT: {
    ...
    if (!param->write.is_prep) {
      ...
      if (ble_write_cb) {
        ble_write_cb(param->write.value, param->write.len);
      }

      // if (gl_profile_tab[PROFILE_A_APP_ID].descr1_handle ==
      //         param->write.handle &&
      //     param->write.len == 2) {
      //   uint16_t descr_value =
      //       param->write.value[1] << 8 | param->write.value[0];
      //   if (descr_value == 0x0001) {
      //     if (read_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY) {
      ...
      //           gl_profile_tab[PROFILE_A_APP_ID].char1_handle,
      //           sizeof(notify_data), notify_data, false);
      //     }
      //   } else if (descr_value == 0x0002) {
      //     if (read_property & ESP_GATT_CHAR_PROP_BIT_INDICATE) {
  ...
  case ESP_GATTS_CREATE_EVT:
    ...
    gl_profile_tab[PROFILE_A_APP_ID].char1_uuid.len = ESP_UUID_LEN_16;
    gl_profile_tab[PROFILE_A_APP_ID].char1_uuid.uuid.uuid16 = GATTS_CHAR1_UUID;
    gl_profile_tab[PROFILE_A_APP_ID].char2_uuid.len = ESP_UUID_LEN_16;
    gl_profile_tab[PROFILE_A_APP_ID].char2_uuid.uuid.uuid16 = GATTS_CHAR2_UUID;

    esp_ble_gatts_start_service(
        gl_profile_tab[PROFILE_A_APP_ID].service_handle);
    // read_property = ESP_GATT_CHAR_PROP_BIT_READ |
    ...
    esp_attr_value_t gatts_char1_val = {
        ...
        .attr_len = ble_prop_buf_len,
        .attr_value = ble_prop_buf,
    };

    esp_err_t add_char_ret = esp_ble_gatts_add_char(
        gl_profile_tab[PROFILE_A_APP_ID].service_handle,
        &gl_profile_tab[PROFILE_A_APP_ID].char1_uuid, ESP_GATT_PERM_READ,
        read_property, &gatts_char1_val, 0);
    if (add_char_ret) {
    ...
    esp_attr_value_t gatts_char2_val = {
        .attr_max_len = GATTS_DEMO_CHAR_VAL_LEN_MAX,
        .attr_len = sizeof(write_buf),
        .attr_value = write_buf,
    };

    add_char_ret = esp_ble_gatts_add_char(
        gl_profile_tab[PROFILE_A_APP_ID].service_handle,
        &gl_profile_tab[PROFILE_A_APP_ID].char2_uuid, ESP_GATT_PERM_WRITE,
        write_property, &gatts_char2_val, 0);
    if (add_char_ret) {
      ESP_LOGE(GATTS_TAG, "add char failed, error code =%x", add_char_ret);
    }
    break;
  ...
  case ESP_GATTS_ADD_CHAR_EVT: {
    ...
    ESP_LOGI(GATTS_TAG,
             "ADD_CHAR_EVT, char_uuid %d, status %d, attr_handle %d, "
             "service_handle %d",
             param->add_char.char_uuid.uuid.uuid16, param->add_char.status,
             param->add_char.attr_handle, param->add_char.service_handle);

    // if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR1_UUID) {
    //   gl_profile_tab[PROFILE_A_APP_ID].char1_handle =
    //       param->add_char.attr_handle;
    //   gl_profile_tab[PROFILE_A_APP_ID].descr1_uuid.len = ESP_UUID_LEN_16;
    //   gl_profile_tab[PROFILE_A_APP_ID].descr1_uuid.uuid.uuid16 =
    ...
    //       gl_profile_tab[PROFILE_A_APP_ID].service_handle,
    //       &gl_profile_tab[PROFILE_A_APP_ID].descr1_uuid,
    ...
    // if (param->add_char.char_uuid.uuid.uuid16 == GATTS_CHAR2_UUID) {
    //   gl_profile_tab[PROFILE_A_APP_ID].char2_handle =
    //       param->add_char.attr_handle;
    //   gl_profile_tab[PROFILE_A_APP_ID].descr2_uuid.len = ESP_UUID_LEN_16;
    //   gl_profile_tab[PROFILE_A_APP_ID].descr2_uuid.uuid.uuid16 =
    //       ESP_GATT_UUID_CHAR_CLIENT_CONFIG;
    // }
  ...
  case ESP_GATTS_ADD_CHAR_DESCR_EVT:
    gl_profile_tab[PROFILE_A_APP_ID].descr1_handle =
...
//         GATTS_NUM_HANDLE_TEST_B); break;
//     case ESP_GATTS_READ_EVT: {
//         ESP_LOGI(GATTS_TAG, "GATT_READ_EVT, conn_id %d, trans_id %" PRIu32 ",
//         handle %d", param->read.conn_id, param->read.trans_id,
//         param->read.handle); esp_gatt_rsp_t rsp; memset(&rsp, 0,
//         sizeof(esp_gatt_rsp_t)); rsp.attr_value.handle = param->read.handle;
...
//     }
//     case ESP_GATTS_WRITE_EVT: {
//         ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, conn_id %d, trans_id %" PRIu32
//         ", handle %d", param->write.conn_id, param->write.trans_id,
//         param->write.handle); if (!param->write.is_prep){
//             ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, value len %d, value :",
//             param->write.len); esp_log_buffer_hex(GATTS_TAG,
//             param->write.value, param->write.len); if
...
//                 param->write.value[0]; if (descr_value == 0x0001){
...
//         break;
//     case ESP_GATTS_CREATE_EVT:
//         ESP_LOGI(GATTS_TAG, "CREATE_SERVICE_EVT, status %d,  service_handle
//         %d", param->create.status, param->create.service_handle);
...
//     case ESP_GATTS_ADD_CHAR_DESCR_EVT:
//         gl_profile_tab[PROFILE_B_APP_ID].descr_handle =
//         param->add_char_descr.attr_handle; ESP_LOGI(GATTS_TAG,
//         "ADD_DESCR_EVT, status %d, attr_handle %d, service_handle %d",
...
//         break;
//     case ESP_GATTS_START_EVT:
//         ESP_LOGI(GATTS_TAG, "SERVICE_START_EVT, status %d, service_handle
//         %d",
//                  param->start.status, param->start.service_handle);
//         break;
//     case ESP_GATTS_STOP_EVT:
//         break;
//     case ESP_GATTS_CONNECT_EVT:
//         ESP_LOGI(GATTS_TAG, "CONNECT_EVT, conn_id %d, remote
//         %02x:%02x:%02x:%02x:%02x:%02x:",
...
//     case ESP_GATTS_CONF_EVT:
//         ESP_LOGI(GATTS_TAG, "ESP_GATTS_CONF_EVT status %d attr_handle %d",
//         param->conf.status, param->conf.handle); if (param->conf.status !=
//         ESP_GATT_OK){
...
void ble_server_init(const ble_server_config *ble_config) {
  ...
  ble_prop_buf = ble_config->prop_buf;
  ble_prop_buf_len = ble_config->prop_len;
  ble_write_cb = ble_config->write_cb;
  ble_read_cb = ble_config->read_cb;
  ...
  if (mac_addr != NULL) {
    ESP_LOGI(GATTS_TAG,
             ...
             mac_addr[5]);
    sprintf(ble_device_name, TEST_DEVICE_NAME "_%02X%02X%02X", mac_addr[0],
            mac_addr[1], mac_addr[2]);
  } else {
    printf("Failed to get Bluetooth MAC address\n");
  }
